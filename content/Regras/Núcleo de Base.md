O **Núcleo de Base** é a estrutura central responsável pela criação, desenvolvimento e administração de uma Base.

Ele funciona como o ponto principal de controle do território e de toda a infraestrutura construída dentro dele.

É através do Núcleo que uma Base pode:

- Reivindicar território.
- Expandir sua área de influência.
- Construir novas estruturas.
- Evoluir estruturas existentes.
- Administrar recursos destinados ao desenvolvimento da Base.
- Desbloquear novas construções conforme seu Tier aumenta.
- Organizar o crescimento da população e da infraestrutura.
- Sustentar sistemas como [[Estátuas de Classe]], Mercados, estruturas militares, produtivas e administrativas.

O Núcleo não cria automaticamente uma cidade completa.

Ele fornece a **estrutura necessária para que uma Base possa ser construída e desenvolvida**.

Uma Base recém-criada pode começar apenas como um pequeno território controlado ao redor do Núcleo e, através de investimento, exploração, recursos e expansão, crescer até se tornar um assentamento, cidade, fortaleza ou até uma estrutura territorial muito maior.

---

## Desenvolvimento da Base

O desenvolvimento do Núcleo é coletivo.

Diferente do desenvolvimento individual de um personagem, sua evolução depende dos recursos acumulados pela própria Base.

Normalmente, construções e melhorias exigem:

> **Gold + Recursos**

Os recursos necessários dependem daquilo que está sendo construído.

Uma estrutura de madeira pode exigir grande quantidade de Madeira, enquanto fortificações, oficinas ou construções avançadas podem exigir Pedra, Metal ou materiais especiais.

---

## Tier do Núcleo

Todo Núcleo possui um **Tier** que representa o nível geral de desenvolvimento da Base.

O Tier determina coisas como:

- Quais estruturas podem ser construídas.
- Até qual Tier determinadas estruturas podem evoluir.
- Quais sistemas da Base estão disponíveis.
- O limite de expansão territorial.
- O nível de infraestrutura que o Núcleo consegue sustentar.

Quanto maior o Tier do Núcleo, maior o potencial de desenvolvimento da Base.

Entretanto:

> **Evoluir o Núcleo aumenta aquilo que a Base pode fazer. Não realiza esse desenvolvimento automaticamente.**

Desbloquear uma nova estrutura não significa que ela já esteja construída.

A Base ainda precisa pagar seus custos, fornecer os recursos necessários e cumprir seus requisitos.

# Estruturas do Núcleo de Base

As **Estruturas** são construções adquiridas através do [[Núcleo de Base]].

Elas representam o desenvolvimento físico e funcional da Base e podem fornecer benefícios econômicos, militares, administrativos, sociais ou produtivos.

Cada estrutura possui:

- Tier.
- Custo em Gold.
- Custo em Unidades de Recursos.
- Função própria.
- Capacidade, quando aplicável.
- Benefícios.
- Estatísticas próprias, quando puder ser atacada ou destruída.
- Requisitos específicos, quando necessário.

> **Construir uma estrutura não significa apenas colocar um prédio no mapa.**
>
> Cada estrutura adiciona uma função mecânica real à Base.

---

# Construção

Estruturas são adquiridas diretamente através do [[Núcleo de Base]].

Seu custo normalmente utiliza:

> **Gold + Unidades de Recursos**

Gold representa o investimento mais raro e valioso.

Os Recursos representam os materiais físicos necessários para realizar a construção.

Entre os recursos mais comuns estão:

- Madeira.
- Pedra.
- Metal.
- Argila.
- Fibra.
- Couro.
- Materiais especiais.

O tipo e quantidade necessária dependem da estrutura.

---

# Tier das Estruturas

Toda estrutura possui um Tier.

Como regra geral:

> **Uma estrutura não pode possuir Tier superior ao Tier do Núcleo de Base.**

Portanto:

> Núcleo Tier 1 → Estruturas até Tier 1  
> Núcleo Tier 2 → Estruturas até Tier 2  
> Núcleo Tier 3 → Estruturas até Tier 3

e assim sucessivamente.

Algumas estruturas podem possuir requisitos adicionais para serem evoluídas.

---

# Posicionamento

Estruturas compradas podem ser posicionadas dentro da **Área Reclamada** pelo Núcleo.

Elas não precisam necessariamente ser construídas próximas ao Núcleo.

Isso permite, por exemplo:

- Torres de Vigia nas fronteiras.
- Postos de Coleta dentro de florestas.
- Fazendas em regiões férteis.
- Fortificações em pontos estratégicos.
- Armazéns próximos de áreas produtivas.
- Estábulos próximos de estradas.

Salvo quando uma estrutura possuir uma regra diferente:

> **Ela precisa permanecer dentro do território controlado pelo Núcleo.**

---

# Estruturas Repetíveis

Algumas estruturas podem ser construídas várias vezes.

Exemplos:

- Moradias.
- Fazendas.
- Torres de Vigia.
- Postos de Coleta.
- Paliçadas.
- Quartéis.
- Estábulos.

Estruturas adicionais normalmente aumentam:

- Capacidade.
- Cobertura territorial.
- Quantidade de trabalhadores.
- Quantidade de habitantes.
- Quantidade de soldados.

Bônus iguais de estruturas idênticas normalmente:

> **não se acumulam sobre o mesmo personagem.**

---

# Estruturas Normais — Tier 1

As Estruturas Tier 1 representam a infraestrutura disponível para uma Base recém-estabelecida.

---

# Mercado — Tier 1

> **Custo:** 75 Gold  
> **Madeira:** 120 Unidades  
> **Pedra:** 60 Unidades  
> **Metal:** 10 Unidades

O Mercado conecta a Base aos outros Mercados existentes dentro da mesma **Região**.

Uma Região representa uma grande divisão territorial.

Como referência:

> Todo o Estado de São Paulo poderia representar uma Região.

---

## Benefícios

O Mercado permite:

- Visualizar ofertas existentes em outros Mercados da Região.
- Comprar produtos anunciados na rede regional.
- Anunciar produtos da própria Base.
- Vender recursos e itens para outros centros comerciais.
- Registrar comerciantes.
- Atrair comerciantes e caravanas.
- Utilizar a Base como ponto oficial de comércio.

Podem fazer parte da rede:

- Vilas.
- Cidades.
- Grandes Cidades.
- Outras Bases.
- Centros comerciais especiais.

> O Mercado conecta os sistemas comerciais.
>
> Isso não significa necessariamente que os produtos sejam transportados instantaneamente.

Transporte e entrega continuam sujeitos às regras apropriadas.

---

# Armazém — Tier 1

> **Custo:** 30 Gold  
> **Madeira:** 100 Unidades  
> **Pedra:** 40 Unidades  
> **Metal:** 5 Unidades

Estrutura responsável pelo armazenamento oficial dos recursos pertencentes à Base.

---

## Capacidade

> **500 Unidades de Recursos**

---

## Benefícios

Recursos armazenados no Armazém podem ser utilizados diretamente pelo Núcleo para:

- Construções.
- Reparos.
- Evoluções.
- Aprimoramentos.
- Produção de outras estruturas.

O Armazém também:

- Organiza automaticamente os recursos armazenados.
- Permite definir quem pode retirar ou adicionar materiais.
- Protege materiais contra exposição comum ao clima.
- Permite visualizar o estoque disponível através do Núcleo.

Recursos guardados fora do Armazém ainda existem fisicamente, mas não fazem parte automaticamente do estoque oficial da Base.

---

# Moradia — Tier 1

> **Custo:** 15 Gold  
> **Madeira:** 60 Unidades  
> **Pedra:** 10 Unidades

Estrutura destinada à habitação permanente da população.

---

## Capacidade

Cada Moradia fornece:

> **10 espaços de habitação**

Porém, a quantidade máxima de habitantes permanentes continua limitada pelo Tier do [[Núcleo de Base]].

### Núcleo Tier 1

> **População Máxima: 30 habitantes**

Portanto, três Moradias Tier 1 são suficientes para abrigar toda a população máxima de um Núcleo Tier 1.

Construir Moradias adicionais não aumenta o Limite Populacional do Núcleo.

---

## Benefícios

Habitantes com Moradia:

- Possuem abrigo adequado.
- Podem realizar [[Regras/Descanso#Descanso Prolongado|Descanso Longo]] adequadamente.
- Evitam penalidades relacionadas à falta de abrigo.
- Podem ser registrados como residentes permanentes.
- Possuem condições adequadas para viver dentro da Base.

Visitantes, comerciantes e tropas temporárias não precisam necessariamente ocupar uma vaga permanente.

---

# Fazenda — Tier 1

> **Custo:** 20 Gold  
> **Madeira:** 50 Unidades  
> **Pedra:** 10 Unidades

Estrutura responsável pela produção organizada de alimentos.

Pode representar:

- Plantações.
- Hortas.
- Pomares.
- Pequenas criações.
- Outros métodos agrícolas compatíveis.

---

## Capacidade

> **5 trabalhadores**

---

## Benefícios

Trabalhadores utilizando a Fazenda recebem:

> **+2 Unidades adicionais de produção agrícola por jornada de trabalho.**

A Fazenda também:

- Permite cultivo permanente.
- Organiza irrigação e armazenamento básico.
- Reduz perdas comuns da produção.
- Permite utilização de Habilidades e Passivas relacionadas à agricultura.
- Permite manter plantações de forma eficiente.

A produção ainda depende de:

- Agricultores.
- Ferramentas.
- Solo.
- Clima.
- Sementes.
- Cultivo utilizado.

O bônus da Fazenda não substitui a produção dos trabalhadores.

---

# Oficina — Tier 1

> **Custo:** 40 Gold  
> **Madeira:** 80 Unidades  
> **Pedra:** 40 Unidades  
> **Metal:** 15 Unidades

Estrutura destinada à fabricação, manutenção e reparação de itens.

---

## Capacidade

> **5 trabalhadores**

---

## Benefícios

Personagens trabalhando dentro da Oficina recebem:

> **+5 em Testes de Perícia Profissional**

enquanto realizarem atividades compatíveis com a estrutura.

O bônus somente se aplica quando a Perícia utilizada realmente puder se beneficiar das ferramentas, bancadas, equipamentos ou espaço fornecidos pela Oficina.

Exemplos de Perícias que podem receber o bônus quando apropriado:

- Ferraria.
- Artesanato.
- Construção.
- Tecnologia Científica.
- Outras Perícias profissionais compatíveis.

### Exemplo

Um Ferreiro utilizando:

> **Ferraria**

para fabricar uma arma dentro da Oficina recebe:

> **+5 no Teste.**

Um Artesão utilizando:

> **Artesanato**

para produzir um item compatível também pode receber:

> **+5 no Teste.**

Por outro lado, atividades que não dependam da infraestrutura da Oficina não recebem esse benefício.

Exemplos:

- cultivar uma plantação;
- rastrear uma criatura;
- colher recursos em uma floresta;
- realizar uma negociação comercial.

A Oficina também:

- Permite fabricar itens que exijam uma estação adequada.
- Permite realizar reparos complexos.
- Fornece ferramentas básicas de produção.
- Permite organização de materiais.
- Libera trabalhos impossíveis de realizar adequadamente de maneira improvisada.

> A Oficina não substitui requisitos de Classe, Perícia, conhecimento, receita ou materiais.

A Oficina também:

> **não concede uma Perícia profissional ao personagem.**

Ela apenas melhora as condições de trabalho de alguém que já possua capacidade adequada para realizar a atividade.

---

# Quartel — Tier 1

> **Custo:** 50 Gold  
> **Madeira:** 120 Unidades  
> **Pedra:** 50 Unidades  
> **Metal:** 10 Unidades

Estrutura destinada à organização e treinamento das forças militares da Base.

---

## Capacidade

> **20 soldados**

---

## Benefícios

Soldados oficialmente vinculados ao Quartel recebem:

> **+3 [[Regras/Defesa|Defesa]] Física**  
> **+3 Defesa Mágica**  
> **+3 [[Regras/Precisão|Precisão]]**  
> **+3 [[Regras/Dano|Dano]]**  
> **+20 [[Regras/Vida|Vida]]**

Os bônus representam:

- Treinamento militar.
- Disciplina.
- Organização.
- Preparação.
- Suporte fornecido pela Base.

O soldado não precisa permanecer dentro do Quartel para manter os benefícios.

---

## Funções

O Quartel também permite:

- Organizar patrulhas.
- Criar turnos de guarda.
- Formar grupos militares.
- Armazenar equipamentos.
- Estabelecer cadeia de comando.
- Manter soldados oficialmente registrados como força militar.
- Mobilizar tropas durante ataques.

Um personagem não recebe bônus adicionais por estar registrado em múltiplos Quartéis.

Quartéis adicionais servem principalmente para aumentar a capacidade militar.

---

# Torre de Vigia — Tier 1

> **Custo:** 15 Gold  
> **Madeira:** 50 Unidades  
> **Pedra:** 10 Unidades

Estrutura elevada destinada à vigilância territorial.

Pode ser construída:

> **Em qualquer local dentro do território reclamado pelo Núcleo.**

---

## Capacidade

> **2 Vigias**

---

## Área de Vigilância

> **500 metros de raio**

Obstáculos podem reduzir a área efetivamente observável.

Exemplos:

- Florestas.
- Montanhas.
- Construções.
- Neblina.
- Escuridão.
- Condições climáticas.

---

## Benefícios

Personagens utilizando a Torre para vigiar recebem:

> **+15 [[Atributos Centrais/Intelecto#Percepção|Percepção]]**

dentro da Área de Vigilância da Torre.

Esse bônus pode ser utilizado para detectar:

- Criaturas.
- Inimigos.
- Movimentação suspeita.
- Exércitos.
- Incêndios.
- Ataques.
- Pessoas se aproximando.
- Alterações visíveis no território.

Duas Torres cobrindo a mesma área:

> **não concedem +30 Percepção.**

O bônus permanece +15.

---

# Paliçada — Tier 1

> **Custo:** 5 Gold por Seção  
> **Madeira:** 30 Unidades por Seção

Estrutura defensiva simples utilizada para proteger áreas da Base.

---

## Estatísticas

> **Vida:** 300  
> **Defesa Física:** 10  
> **Defesa Mágica:** 2

---

## Benefícios

A Paliçada:

- Impede passagem terrestre comum.
- Obriga inimigos a destruir, escalar ou contornar a defesa.
- Cria cobertura física.
- Permite delimitar áreas protegidas.
- Pode ser utilizada para canalizar inimigos em direção a entradas específicas.

Cada compra corresponde a uma Seção de Paliçada.

---

# Portão Fortificado — Tier 1

> **Custo:** 15 Gold  
> **Madeira:** 50 Unidades  
> **Metal:** 10 Unidades

Entrada defensiva utilizada junto a Paliçadas.

---

## Estatísticas

> **Vida:** 400  
> **Defesa Física:** 12  
> **Defesa Mágica:** 2

---

## Benefícios

O Portão:

- Permite criar entradas controladas.
- Pode ser aberto e fechado por pessoas autorizadas.
- Impede passagem enquanto estiver fechado.
- Permite controlar entrada de habitantes, comerciantes e visitantes.
- Pode receber guardas designados.

Personagens não autorizados precisam destruir, escalar, contornar ou utilizar algum outro método para atravessá-lo.

---

# Poço — Tier 1

> **Custo:** 10 Gold  
> **Madeira:** 10 Unidades  
> **Pedra:** 40 Unidades

Estrutura responsável por fornecer acesso organizado à água subterrânea.

---

## Benefícios

Um Poço funcional fornece água para:

> **Até 30 habitantes**

Também pode ser utilizado para:

- Animais.
- Fazendas.
- Oficinas.
- Limpeza.
- Produção básica.

Enquanto houver água disponível, o Núcleo Tier 1 pode utilizar um único Poço para abastecer sua população máxima.

> O Poço precisa ser construído em um local adequado.

Se não existir água subterrânea acessível, a estrutura não funcionará normalmente.

---

# Estábulo — Tier 1

> **Custo:** 20 Gold  
> **Madeira:** 70 Unidades  
> **Pedra:** 10 Unidades

Estrutura destinada a montarias e animais utilizados pela Base.

---

## Capacidade

> **10 animais de porte médio**

Criaturas maiores podem ocupar mais espaços.

---

## Benefícios

Animais registrados no Estábulo:

> **+20 [[Regras/Estamina|Estamina]] Máxima durante viagens iniciadas após um Descanso Longo no Estábulo.**

Além disso:

- Possuem abrigo adequado.
- Podem recuperar-se corretamente durante descanso.
- Equipamentos de montaria podem ser armazenados.
- Alimentos podem ser organizados.
- Animais de carga podem ser administrados pela Base.

---

# Praça Central — Tier 1

> **Custo:** 10 Gold  
> **Madeira:** 10 Unidades  
> **Pedra:** 40 Unidades

Área pública destinada à organização social da Base.

---

## Benefícios

Enquanto estiver realizando uma atividade pública organizada na Praça, a liderança recebe:

> **+5 Liderança**

A Praça também permite:

- Realizar anúncios.
- Reunir habitantes.
- Organizar eventos.
- Fazer discursos.
- Distribuir recursos.
- Publicar trabalhos disponíveis.
- Realizar recrutamento.
- Organizar reuniões públicas.

A Praça funciona como principal ponto de interação entre liderança e população no Tier 1.

---

# Posto de Coleta — Tier 1

> **Custo:** 20 Gold  
> **Madeira:** 60 Unidades  
> **Pedra:** 10 Unidades  
> **Metal:** 5 Unidades

Estrutura utilizada para organizar a coleta de um recurso natural específico existente dentro do território.

Ao ser construído, o Posto deve ser vinculado a um recurso.

### Exemplos

- Madeira.
- Pedra.
- Argila.
- Fibra.
- Plantas.
- Recursos naturais comuns.

---

## Capacidade

> **5 trabalhadores**

---

## Benefícios

Cada trabalhador utilizando o Posto recebe:

> **+1 Unidade adicional do recurso coletado por jornada de trabalho.**

Além disso:

- Recursos podem ser enviados diretamente para um Armazém conectado.
- Ferramentas podem ser mantidas no local.
- Trabalhadores podem ser organizados em equipes.
- A Base pode controlar oficialmente a exploração daquela fonte.

O Posto:

> **não cria recursos.**

Precisa existir uma fonte natural válida dentro de sua área de atuação.

---

# Fontes Naturais

Recursos naturais possuem seus próprios limites.

Uma floresta não suporta quantidade infinita de Lenhadores.

Uma mina não produz minério ilimitado.

Uma fonte pode possuir diferentes escalas:

- Pequena.
- Média.
- Grande.
- Excepcional.

Quanto maior a fonte:

- Maior sua reserva.
- Mais trabalhadores ela suporta.
- Maior sua capacidade de produção.

Exploração excessiva pode:

- Reduzir rendimento.
- Esgotar o recurso.
- Destruir permanentemente a fonte.
- Exigir tempo para recuperação, quando aplicável.

---

# Limites de Trabalhadores

Estruturas produtivas possuem uma capacidade máxima de trabalhadores.

Exceder essa capacidade:

> **não aumenta automaticamente a produção.**

Para utilizar mais trabalhadores será necessário:

- Construir outra estrutura.
- Evoluir a estrutura.
- Utilizar uma instalação maior.

Isso impede que uma única estrutura Tier 1 seja utilizada por dezenas de trabalhadores simultaneamente.

---

# Resumo — Estruturas Tier 1

|Estrutura|Gold|Madeira|Pedra|Metal|Principal Benefício|
|---|---|---|---|---|---|
|Mercado|75|120|60|10|Comércio Regional|
|Armazém|30|100|40|5|500 U de armazenamento|
|Moradia|15|60|10|—|10 habitantes|
|Fazenda|20|50|10|—|+2 produção agrícola|
|Oficina|40|80|40|15|+5 em Testes de Perícia Profissional compatíveis|
|Quartel|50|120|50|10|Bônus militares|
|Torre de Vigia|15|50|10|—|+15 Percepção|
|Paliçada|5|30|—|—|Fortificação|
|Portão|15|50|—|10|Entrada fortificada|
|Poço|10|10|40|—|Água para 30 habitantes|
|Estábulo|20|70|10|—|Suporte a 10 animais|
|Praça Central|10|10|40|—|+5 Liderança|
|Posto de Coleta|20|60|10|5|+1 U por trabalhador|

---

# Limites do Núcleo Tier 1

Um [[Núcleo de Base]] Tier 1 possui como referência:

> **População Máxima: 30 habitantes**

As estruturas construídas não podem ignorar esse limite.

Por exemplo:

> 5 Moradias = capacidade física para 50 habitantes.

Porém:

> Núcleo Tier 1 = máximo de 30 habitantes permanentes.

Para aumentar esse limite será necessário evoluir o próprio Núcleo.

---

# Filosofia das Estruturas

Estruturas não substituem personagens ou Classes.

Uma Fazenda sem Agricultores ainda precisa de pessoas capazes de trabalhar nela.

Uma Oficina não transforma qualquer pessoa em artesão.

Um Quartel não concede automaticamente uma Classe militar.

Uma Torre não enxerga sozinha.

> **Estruturas fornecem infraestrutura e benefícios.**
>
> **Pessoas fazem essa infraestrutura funcionar.**

Quanto mais desenvolvida a Base, maior será a interação entre:

- Estruturas.
- Classes.
- Trabalhadores.
- Recursos.
- População.
- Território.
- Liderança.
